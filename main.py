...
def generate_mitigation_plan(data: ParcelInput):
    ...
    # Configuration to enforce professional English and strict brevity
    ...
    # 3. Call Gemini API with automatic fallback for 503 capacity issues
    try:
        ...
        print(f"Primary model busy ({primary_err}), switching to fallback model...")
        try:
            ...
            raise HTTPException(
                status_code=503, 
                detail="Gemini services are currently experiencing high global load. Please retry in a few seconds."
            )
